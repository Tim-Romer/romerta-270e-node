...
function getGame(gameId) {
	...
	return new Promise(function(resolv,reject) {
		game.findOne({gameId: gameId},function(error, record) {
			if (error) {
				debugLog("on play, existig id error",error);
				console.log(error);
				reject(error)
			} else
			{
	...
	return new Promise(function(resolv,reject) {
		if (gameId==="0") {
			...
			createId().then((id) => {
				...
				var rec = new game({gameId: id,player1Name: playerName,player2Name:"",nextMove:1,moveCnt:0,lastMoveTime: time,board:[0,0,0,0,0,0,0,0,0],state:"waiting"});
//				console.log(rec);
                                rec.save().then(() => {
					resolv(rec);
				});
...
function clear(date) {
	return new Promise(function(resolv, reject) {
		console.log("Games ready to clear");
